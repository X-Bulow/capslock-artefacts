...
\hline
{\footnotesize\color{mute}Toolchain commit} & \rule{0pt}{4.4mm}Artefact pins plus local patches; see A1 evidence \\
\hline
{\footnotesize\color{mute}Branch / worktree} & \rule{0pt}{4.4mm}Published artefact snapshot (not a Git worktree) \\
\hline
\end{tabular}
...
\noindent{\footnotesize\color{mute}Status:}\ \texttt{[\ ]}~not started \quad
  \texttt{[\ ]}~in progress \quad \texttt{[X]}~done \quad \texttt{[\ ]}~blocked\par\vspace{3pt}
\begin{answerbox}{A2}
Nothing repoints Rust's \texttt{src/llvm-project} submodule. The artefact
bypasses it. \texttt{setup/setup.py} fetches the pinned upstream LLVM tree and
...
Before G1, append from the x86-64 host: CPU/OS, exact commands,
workload revision and input hash, build flags, QEMU revisions,
warm-up policy, and the five raw samples for every variant.
\end{verbatim}
\end{answerbox}
