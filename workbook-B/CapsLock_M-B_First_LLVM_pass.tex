...
\hline
{\footnotesize\color{mute}Date submitted} & \rule{0pt}{4.4mm} TODO \\
\hline
...
\end{askbox}
\noindent{\footnotesize\color{mute}Status:}\ \texttt{[\ ]}~not started \quad
  \texttt{[x]}~in progress \quad \texttt{[\ ]}~done \quad
  \texttt{[\ ]}~blocked\par\vspace{3pt}
\begin{answerbox}{B4}
Work in progress.  I am learning how LLVM IR represents memory accesses,
with particular attention to \texttt{LoadInst}, \texttt{StoreInst}, their
pointer operands, and using the module's \texttt{DataLayout} to obtain the
store size of the accessed type.  I am also studying how
\texttt{getelementptr} identifies a structure field so that the reported
size describes the field being accessed rather than the entire structure.

The next implementation step is to enumerate loads and stores and log their
addresses and byte sizes, then verify the results for \texttt{i8},
\texttt{i32}, \texttt{i64}, and a structure-field access.  Atomic operations
and the \texttt{memcpy}, \texttt{memmove}, and \texttt{memset} intrinsics
will be recorded explicitly as memory-access categories not yet handled by
this B4 prototype.
...
\end{askbox}
\noindent{\footnotesize\color{mute}Status:}\ \texttt{[x]}~not started \quad
  \texttt{[\ ]}~in progress \quad \texttt{[\ ]}~done \quad
  \texttt{[\ ]}~blocked\par\vspace{3pt}
\begin{answerbox}{B5}
%% Write your answer here.
